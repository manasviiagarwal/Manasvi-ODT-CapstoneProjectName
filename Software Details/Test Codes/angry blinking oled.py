...
def draw_angry_brows():
    oled.line(28, 10, 52, 18, 1)    # left brow  \
    oled.line(76, 18, 100, 10, 1)   # right brow /

def draw_frown():
    draw_arc(64, 52, 10, 200, 340)  # upside down arc

# ── Main blink loop ──────────────────────────────────────

def angry_blink_forever():
    while True:
        # eyes open
        oled.fill(0)
        eyes_open()
        ...
        oled.fill(0)
        eyes_closed()
        draw_angry_brows()          # brows stay during blink
        draw_frown()
        oled.show()
